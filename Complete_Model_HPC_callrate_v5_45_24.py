...
class SCDataset(Dataset):
    def __init__(self, root, transform=None, pre_transform=None, pre_filter=None):
        ...
        self.call_rate = self.calculate_call_rate_by_node().to(device) #this is the call rate for the entire dataset. it's a tensor of size [num_nodes]
    ...
    def get_without_call_rates(self, idx): #this is referenced only once by the call rate method during initialization.
        data = torch.load(osp.join(self.root, self.raw_file_names[idx]))
        # skip call_rates line here
        data.edge_index = self.edge_index.to(device)
    ...
    def get(self, idx):
        data = torch.load(osp.join(self.root, self.raw_file_names[idx]))
        data = data.to(device)
        call_rate = self.call_rate # retrieve the pre-computed call rates
        data.x = torch.cat((data.x, call_rate.unsqueeze(-1)), dim=-1)  # Append call rate as a new feature
        data.edge_index = self.edge_index.to(device)
        if data.x.device != device:
            data = data.to(device)
        return data
    ...
    def calculate_call_rate_by_node(self):
        ...
        node_nonzero_count = torch.zeros(self.get_without_call_rates(0).num_nodes, dtype=torch.float).to(device)  # non-zero count per node
        if synth_data == False:
        ...
        #sparsify the call rate
        non_zero_indices = call_rate.nonzero(as_tuple=False).t() # Get the indices in COO format
        sparse_call_rate = torch.sparse_coo_tensor(indices=non_zero_indices, 
                                               values=call_rate[call_rate.nonzero(as_tuple=True)], 
                                               size=call_rate.size()).to(device)
...
class GAT(torch.nn.Module):
    def __init__(self, hidden_channels):
        ...
        torch.manual_seed(12345)
        self.conv1 = GATConv(dataset.num_node_features, hidden_channels, heads=num_heads)
        self.conv2 = GATConv(hidden_channels*num_heads, hidden_channels, heads=num_heads)
        self.pool1 = SAGPoolingCustom(hidden_channels*num_heads, min_score=None, multiplier=1.0, ratio = pooling_keep_count)
        self.lin = Linear(hidden_channels*num_heads, num_classes) #I had dataset.num_classes here, but it loads every data object at once to find out how many classes there are which caused GPU death. instead made a global variable num_classes 
        self.batch_norm1 = torch.nn.BatchNorm1d(hidden_channels*num_heads)
        self.batch_norm2 = torch.nn.BatchNorm1d(hidden_channels*num_heads)
    ...
    def forward(self, x, edge_index, batch):
        ...
        #pre_drop_score_two = abs(pre_drop_score_two)
...
def test(loader):
    ...
    for data in loader:
        ...
        if epoch == num_epochs: #ie if it's the last epoch. 
            ...
            assert len(split_pre_drop_scores) == num_graphs_in_batch, f"Not all values from the original tensor were used, expected {batch_size} tensors, got {len(split_pre_drop_scores)}"
            """
            predrop_by_epoch_list.append({epoch: single_batch_scoresum.cpu().numpy()}) #sent to cpu because numpy can't handle gpus. unknown if this will cause problems.
...
#dense up the call rate for saving
